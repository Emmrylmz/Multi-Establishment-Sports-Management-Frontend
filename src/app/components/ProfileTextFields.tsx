...
const ProfileTextFields = ({label,value,editable,isPassword,handleChangeText}: ProfileTextFieldsProps) => {
  ...
  if(editable){
    return (
      ...
      <TextInput value={value} className='text-base text-white' onChangeText={(text) => handleChangeText(text,label)} secureTextEntry={isPassword ? true : false} editable={isEditable} />
        <TouchableOpacity onPress={() => setIsEditable((prevState) => !prevState)}>
